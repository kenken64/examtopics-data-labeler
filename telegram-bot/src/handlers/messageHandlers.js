...
class MessageHandlers {
  ...
  async handleAccessCodeSubmission(ctx, accessCode, userSessions) {
    ...
    try {
      const questions = await this.quizService.getQuestionsForAccessCode(accessCode, session.certificateId);
      ...
      const certificateDisplay = certificate ? `${certificate.name} (${certificate.code})` : 'Unknown Certificate';
  ...
  async showCurrentQuestion(ctx, userSessions, userSelections = new Map()) {
    ...
    const keyboard = this.quizService.createQuestionKeyboard(currentQuestion, currentUserSelections);
  ...
  async handleBookmark(ctx, userSessions) {
    ...
    if (isNaN(questionNumber) || questionNumber < 1 || questionNumber > session.questions.length) {
      await ctx.reply(`❌ Invalid question number. Please enter a number between 1 and ${session.questions.length}.`);
      return;
    }

    try {
      const questionToBookmark = session.questions[questionNumber - 1];
      await this.databaseService.saveBookmark(userId, questionToBookmark._id, questionNumber, session.accessCode);
  ...
  async handleBookmarks(ctx, userSessions) {
    ...
    try {
      ...
      if (currentBookmarks.length === 0) {
        await ctx.reply('📑 No bookmarks found for current access code. Use /bookmark <number> to save questions.');
        return;
      }
