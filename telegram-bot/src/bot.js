...
class CertificationBot {
  ...
  async handleJoinQuizByCode(ctx, quizCode) {
    try {
      ...
      console.log(`🎮 User ${playerName} (@${username}, ID: ${telegramUserId}) trying to join quiz ${quizCode}`);
  ...
  async handleQuizAnswer(ctx, selectedAnswer, quizCode) {
    try {
      ...
      console.log(`📝 User ${playerName} (@${username}) selected answer ${selectedAnswer} for quiz ${quizCode}`);
      ...
      console.log(`✅ Answer ${selectedAnswer} submitted for user ${playerName} - ${isCorrect ? 'CORRECT' : 'WRONG'} (${score} points)`);
