...
class QuizService {
  ...
  createQuestionKeyboard(currentQuestion, _userSelections = []) {
    ...
    // Multiple answer layout with confirm/clear buttons
      ...
      // Add control buttons
      keyboard
        .text('✅ Confirm Answer', 'confirm_answer')
        .row()
        .text('🔄 Clear Selection', 'clear_selection')
        .row();
    } else {
  ...
  formatQuestionText(
    ...
    '\n\n' +
    (isMultiple
      ? `⚠️ Multiple answers required: Select ${normalizeAnswer(currentQuestion.correctAnswer).length} options`
      : '💡 Select one answer');
